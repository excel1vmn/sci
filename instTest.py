...
# m1 = Metro().play()
# m1.ctrl()
# n1 = Notein(poly=8, scale=0, first=0, last=127, channel=1) 
a1 = Synth(transpo, hfdamp, lfofreq, SIGSNB[0], SIGSNB[8], SIGSNB[16], channel=1, mul=MULPOW[0]).out()

a2 = FreakSynth(transpo, hfdamp, lfofreq, SIGSNB[1], SIGSNB[9], SIGSNB[17], channel=1, mul=MULPOW[1]).out()

a3 = Simpler(snds[3], transpo, hfdamp, lfofreq, SIGSNB[2], SIGSNB[10], SIGSNB[18], channel=2, mul=MULPOW[2]).out()

a4 = WaveShape(snds[9], transpo, hfdamp, lfofreq, SIGSNB[3], SIGSNB[11], SIGSNB[19], channel=2, mul=MULPOW[3]).out()

# # p1 = Pads(sndsSB[6], transpo, hfdamp, channel=10).out()
# # d1 = Drums(kicks, transpo, hfdamp, channel=10).out()

# #Cause underrun
r1 = ReSampler(a1.sig()+a2.sig()+a3.sig()+a4.sig(), TRIGS[0], transpo, SIGSNB[4], SIGSNB[12], SIGSNB[20], channel=1, mul=MULPOW[4]).out()
r2 = ReSampler(a1.sig()+a2.sig()+a3.sig()+a4.sig()+r1.sig(), TRIGS[1], transpo, SIGSNB[5], SIGSNB[13], SIGSNB[21], channel=2, mul=MULPOW[5]).out()
r3 = ReSampler(a1.sig()+a2.sig()+a3.sig()+a4.sig()+r1.sig()+r2.sig(), TRIGS[2], transpo, SIGSNB[6], SIGSNB[14], SIGSNB[22], channel=1, mul=MULPOW[6]).out()
r4 = ReSampler(a1.sig()+a2.sig()+a3.sig()+a4.sig()+r1.sig()+r2.sig()+r3.sig(), TRIGS[3], transpo, SIGSNB[7], SIGSNB[15], SIGSNB[23], channel=2, mul=MULPOW[7]).out()
# #Cause underrun
