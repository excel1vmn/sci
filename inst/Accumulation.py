...
class Accumulation(PyoObject):
    ...
    def __init__(self, input, cs, delay=1, outs=2, mul=1, add=0):
        ...
        self._rand = SigTo(RandDur(min=[self._onesample,self._onesample*1.04,self._onesample*1.09,self._onesample*1.13],max=[.31,.33,.36,.4], mul=self._fade))
        self._del1 = Delay(in_fader, delay=[delay[0]*1.02,delay[0]*2.1,delay[0]*2.8,delay[0]*4.2], feedback=.51, mul=cs[0])
        self._del2 = Delay(in_fader, delay=[delay[0]*.98,delay[0]*1.8,delay[0]*3.3,delay[0]*3.9], feedback=.49, mul=cs[0])
