...
class ReSampler:
    def __init__(self, noteinput, audioREC, trig, toggles, cs, transpo=1, hfdamp=7000, audioIN=0, mul=1):
        ...
        self.tfon = TrigFunc(self.note["trigon"], self.noteon, arg=list(range(10)))
        self.tfoff = TrigFunc(self.note["trigoff"], self.noteoff, arg=list(range(10)))
        ...
        self.envGen = TrigFunc([self.note['trigon'],self.seq], self.createPoints, arg=8).stop()
        self.ind = CosTable([(0,0), (8191,0)], size=8192)
        self.trMod = Osc(self.ind, self.nt.getRate() * self.tra, mul=self.cs[0]).mix().stop()
        ...
        self.dur1 = Choice(choice=[.3125,.0625,.125,.125,.25,.33,.66,2], freq=self.tra)
        self.dur2 = self.nt.getDur()
        self.loop = Looper(self.nt, (self.pitch1+self.pitch2)*self.tra, start=self.start, dur=self.dur1+self.dur2, startfromloop=False, autosmooth=False, mul=self.amp+self.trigenv).mix().stop()
        # PITCH RANDOMIZER #
        ...
        self.dist = Disto(self.lookShape, drive=self.cs[1], slope=self.trMod).mix()
        ...
        self.fs1 = FastSine(freq=self.trMod, quality=0, mul=.5, add=.5)
        self.refSine = FastSine(freq=400).mix()
